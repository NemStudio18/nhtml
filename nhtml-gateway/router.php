...
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_SERVER["CONTENT_TYPE"]) && $_SERVER["CONTENT_TYPE"] === "application/octet-stream") {
    $rawInput = file_get_contents("php://input");
    if (strlen($rawInput) >= 5) {
        $type = ord($rawInput[0]); // [0x02][NodeID:4]
        if ($type === 0x02) {
            ...
            curl_setopt($ch, CURLOPT_POSTFIELDS, json_encode([
                ...
                "current_state" => ["counter_value" => "0"] // POC: État simplifié pour le fallback
            ]));
            ...
            echo pack("N", strlen($payload) + 2); // Len (Payload + 2 bytes for Count)
